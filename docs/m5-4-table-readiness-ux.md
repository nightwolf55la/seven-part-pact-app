...
7. **SOURCE:** the ordinary/default Beast action is still move to an adjacent Sea or Nest on an adjacent Isle. **APPLICATION DESIGN:** adjacency is a strong visual recommendation, not a hard UI legality gate. A Distrusting Beast may drag to any other representable Sea/Horizon (`moveMarinerBeast`) or any structurally valid board Isle (`nestMarinerBeast`). Adjacent destinations receive a stronger highlight; other coherent destinations stay available but quieter. Invalid Nest targets (Market present, Ravaged, existing Nesting Beast) remain blocked. Right-click Recommended Move/Nest lists every adjacent destination from static topology; blocked adjacent Isles stay visible and disabled with a concise reason. Compact **Move elsewhere…** / **Nest elsewhere…** actions open action-triggered choosers for nonadjacent destinations. A required Beast-move Rampage opens one board-local Domain chooser and keeps the pointerdown or menu-open snapshot. SOURCE does not define voluntary Friendly/Nesting Beast relocation. The approved semantic command `relocateMarinerNestingBeast` is APPLICATION DESIGN table-authoritative piece manipulation, not a printed Mariner action. Frontend Isle-marker drag wiring remains the next body.
8. **APPLICATION DESIGN:** the supply tray has two related pieces — Market (`{ present: true, rarity: null }`) and Rare Market (`{ present: true, rarity: non-null }`). Rare Market is UI shorthand for a Market with Rarity, not a new game entity. Ordinary supply Market places on an empty valid Isle or transforms an existing Rare Market to ordinary via one `setMarinerIsleMarket`. Rare supply Market opens a Describe Rarity prompt on empty or ordinary targets and does not mutate until Confirm. Existing Markets drag Isle-to-Isle with one `moveMarinerMarket` (not a remove/add pair). Click inspects the Isle; right-click remains the explicit Add/Edit/Remove Rarity and Remove Market path. No Gift, seasonal, Stability, Wind, Weather, or Ravage automation.
...
- **Resolution / implementation note:** Always-visible: Route occupancy/markers, Storm/Typhoon pieces, Markets (ordinary vs Rare Market star/cue), Ravage silhouette plus compact count, Beast tokens, and immediate-hazard rings on threatened occupied Routes. Map Stability numbers and authoritative prevailing Wind are not encoded in current domain/state and are not implemented. The hollow Visions forecast strip was removed. Beast tokens support Sea-to-Sea move and Isle Nest by drag; adjacent destinations are strongly recommended, other representable/structurally valid destinations remain available but quieter. Contextual Add/Remove remains. Supply tray offers Market and Rare Market. Existing Markets drag with `moveMarinerMarket`. Rarity remains an attribute of the Market, not a separate board piece. Nothing VERIFIED. No broad automation.
...
- Direct Distrusting-Beast Sea move / Nest drag treats adjacency as a strong recommendation; other representable/structurally valid destinations remain available. Contextual Beast Add/Remove, Market / Rare Market supply, existing Market drag (`moveMarinerMarket`), and Market/Rarity context actions are implemented. Rare Market is a Market with Rarity, not a new game entity. No broad gameplay automation.
...
- Undescribed Rare Market uses one reserved application sentinel (`__7PP_APP_UNDESCRIBED_MARINER_RARITY_V1__`) as pre-activation metadata in the existing `rarity` string field. It is not fictional prose and must never be displayed as such. Actual user description later replaces it normally. This representation MUST be reviewed before V5 activation; no migration is approved now.
...
| Nesting Beast relocation semantics | *(this commit)* | M5.4 UX B2: add Nesting Beast relocation semantics |
